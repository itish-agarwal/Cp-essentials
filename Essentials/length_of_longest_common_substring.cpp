...
int main() {
  ...
  auto belong_to_different = [&](int in1, int in2) {
    return (in1 < P && in2 >= P + 1) || (in2 < P && in1 >= P + 1);
  };

  for(int i = 0; i <= N; i++) {
    int in1 = suff.p[i], in2 = suff.p[i + 1];
    if(belong_to_different(in1, in2)) {
      ans = max(ans, suff.lcp[i]);
    }
  }
